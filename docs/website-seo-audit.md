...
| 站外权威度 | 15/100 | 15/100 | 新域名尚未上线、收录和获得外链 |
...
本地 Lighthouse 只能作为方向性数据。Cloud Run 上线后，应以 PageSpeed Insights 实际数据和 Search Console Core Web Vitals 为准。
...
2. GitHub Actions 使用过时的 SiteGround 静态部署。
   - 已修复：工作流目前只做类型检查和生产构建。
   - Cloud Run 部署将在生产项目、区域和密钥确认后单独增加。
...
- 部署网站、验证邮件、提交 sitemap，并请求核心页面收录。
- 建立产品词的展现、点击和排名基线。
- 增加真实公司身份、工厂和运营照片。
